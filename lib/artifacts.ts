 ...
 * ProgramPlanArtifact
 *
 * Structured representation of an office meal program.
 * This is intentionally generic so it can back multiple UIs
 * (office manager dashboard, insights, workflows, etc.).
 */
export const ProgramPlanArtifactSchema = z.object({
  ...
  officeId: z.string(),
  timeRange: z.object({
    startDate: z.string().describe("ISO date (YYYY-MM-DD) when the program starts"),
    endDate: z.string().describe("ISO date (YYYY-MM-DD) when the program ends"),
  }),
  budgetPerPerson: z
    .number()
    .positive()
        ...
        meals: z.array(
          z.object({
            ...
            vendorHint: z
              .string()
              .optional()
              .describe("Optional vendor/restaurant hint the planner had in mind"),
  ...
  notes: z
    .string()
    .optional()
    .describe("Free-form rationale, tradeoffs, or implementation notes for this plan"),
});
