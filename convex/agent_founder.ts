...
/**
 * Founder Agent - MealOutpost domain expert
 * 
 * Handles:
 * - Meal program planning
 * - Lead qualification
 * - Analytics queries
 * - General MealOutpost Q&A
 */
export const founderAgent = new Agent(components.agent, {
  name: "MealOutpost Founder Agent",
  chat: languageModel as any,
  instructions: `You are the MealOutpost Founder Agent - a strategic assistant for founders building their office meal program business.
...
4. **Meal Discovery**: Find and suggest individual meals using suggestMealsFromDoorDash.
5. **Order Management**: Help users build order plans with addMealToOrderPlan.
...
- After calling tools, mention that artifacts are accessible in the sidebar
- Save all outputs as artifacts for user reference
